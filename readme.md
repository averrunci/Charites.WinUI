...
![Charites.WinUI overview](Images/mvcsummary.png)

In Charites.WinUI, the roles of a model, view, and controller are redefined as follows.
...
A controller can be created with a factory that implements IWinUIControllerFactory. If a factory is not specified, a default factory that creates a controller with Activator.CreateInstance method is used.

``` csharp
class ControllerFactory : IWinUIControllerFactory {...}
```

``` csharp
WinUIController.ControllerFactory = new ControllerFactory();
```
