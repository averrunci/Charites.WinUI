...
public class DataTypeDataTemplateSelector : DataTemplateSelector
{
    ...
    protected override DataTemplate SelectTemplateCore(object? item)
        => item is null ? base.SelectTemplateCore(null) : FindDataTemplate(item) ?? base.SelectTemplateCore(item);
    ...
    protected override DataTemplate SelectTemplateCore(object? item, DependencyObject container)
        => item is null ? base.SelectTemplateCore(item, container) : FindDataTemplate(item, container as FrameworkElement) ?? base.SelectTemplateCore(item, container);

    private DataTemplate? FindDataTemplate(object item, FrameworkElement? container = null)
    {
        var targetContainer = container;

        while (targetContainer is not null)
        {
            var dataTemplate = FindDataTemplate(item, targetContainer.Resources);
            if (dataTemplate is not null) return dataTemplate;
    ...
    private DataTemplate? FindDataTemplate(object item, ResourceDictionary? resources) =>
        resources is null ? null : FindDataTemplate(item.GetType(), resources);

    private DataTemplate? FindDataTemplate(Type? dataType, ResourceDictionary resources) =>
        dataType is null ? null : FindDataTemplate(dataType.Name, resources) ??
    ...
    private static string GetFullNameWithoutParameters(Type type)
    {
        ...
        return parameterStartIndex < 0 ? string.Empty : dataTypeFullName[..parameterStartIndex];
    }
