...
public static class KeyRoutedEventArgsWrapper
{
    ...
    /// <returns>
    /// A unique identifier for the input device associated with the key event, or an empty string for an unsupported device.
    /// The same device can be assigned a different ID each time it is connected.
    /// </returns>
