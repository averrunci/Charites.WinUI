...
public static class PointerRoutedEventArgsWrapper
{
    ...
    /// <summary>
    /// Gets a value that marks the routed event as handled, and prevents most handlers along the event route from handling the same event again.
    /// </summary>
    ...
    /// Any UIElement -derived object that is connected to the same object tree.
    /// To specify the object relative to the overall coordinate system, use a <paramref name="relativeTo"/>  value of <c>null</c>.
    /// </param>
    /// <returns>
    /// A PointerPoint value that represents the pointer point associated with this event. If <c>null</c> was passed as <paramref name="relativeTo"/>,
    /// the coordinates are in the frame of reference of the overall window. If a <paramref name="relativeTo"/> value other than <c>null</c> was passed,
    /// the coordinates are relative to the object referenced by <paramref name="relativeTo"/>.
    /// </returns>
    ...
    /// <summary>
    /// Retrieves a collection of PointerPoint objects that represent the pointer history from the last pointer event up to and including the current pointer event.
    /// Each PointerPoint in the collection provides basic info on the pointer associated with the event.
    /// The last item in the collection is equivalent to the PointerPoint object returned by GetCurrentPoint.
    /// </summary>
    /// <param name="e">The requested <see cref="PointerRoutedEventArgs"/>.</param>
    /// <param name="relativeTo">
    /// Provides the context for the pointer data. Can be any UIElement -derived object that is connected to the same object tree.
    /// If <c>null</c>, location coordinates are in the context of the app.
    /// </param>
    /// <returns>
    /// The collection of PointerPoint objects corresponding to the pointer history associated with the event.
    ...
    /// </returns>
    public static IList<PointerPoint> GetIntermediatePointsWrapped(this PointerRoutedEventArgs e, UIElement? relativeTo) => Resolver.GetIntermediatePoints(e, relativeTo);
