...
{
    protected async Task<Window> EnsureWindowAsync() => window ?? await CarnaWinUIRunner.InvokeAsync(() => window = new Window());
    private Window? window;

    protected Task RunAsync(DispatcherQueueHandler action) => CarnaWinUIRunner.InvokeAsync(action);
